...
st.write("사람들이 적절한 개인 보호 장비를 착용하고 있는지 감지합니다")

# Model selection in sidebar (outside of modes)
st.sidebar.header("모델 선택")
model_type = st.sidebar.radio(
    "감지 모델 선택:",
    ["YOLOv11", "RT-DETR v1", "YOLOv11 vs RT-DETR v1 (이미지만 가능)"],
    help="사용할 모델을 선택하세요. 비교 모드는 이미지 업로드에서만 작동합니다."
)
...
if mode == "이미지 업로드":
    ...
    if comparison_mode:
        ...
        st.info("💡 팁: 아래 버튼을 눌러 두 모델의 실시간 성능과 감지 정확도를 동시에 비교할 수 있습니다!")
